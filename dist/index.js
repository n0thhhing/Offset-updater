  ...
  try {
    ...
    return data
      ...
      .map((line) => {
        ...
        return { offset: parseInt(offsetStr.trim(), 16), name: name };
      });
  } catch (error) {
...
function findClosestMatch(segment, patternBytes, firstCharacter) {
  ...
  return { closestMatch: closestMatch, iterationCount: iterationCount };
}
...
async function findOffsetsInNewLibrary(
      ...
      try {
        ...
        while (iterationCount < MAX_ITERATIONS) {
          ...
          if (closestMatch) {
            ...
            if (CHECK_TYPE) {
              ...
              if (oldType && newType) {
                if (
                  ...
                  console.log(
                    chalk.green(
                      "[TYPE_STATUS] - Passed { " + oldType.methodType,
                      oldType.returnType + "}",
                    ),
                  );
                }
            ...
            if (LOGGING) {
              const elapsedTime = (endTime[0] * 1e3 + endTime[1] / 1e6).toFixed(
                3,
              );
              console.log(
                chalk.green(
  ...
  const elapsedTime = cpuEnd.user / 1e3;
  if (LOGGING) {
    console.log(
      chalk.gray(
...
async function main() {
  try {
    ...
    if (LOGGING) {
      const endTime = process.hrtime(startTime);
      const elapsedTime = (endTime[0] * 1e3 + endTime[1] / 1e6).toFixed(2);
      console.log(
