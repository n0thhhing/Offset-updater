...
class KMPBytePatternScanner implements BytePatternScanner {
    ...
    private kmpSearch(buffer: Buffer, pattern: (number | null)[], lps: Int32Array): number[] {
    ...
    scan(buffer: Buffer, pattern: Pattern): number[] {
        ...
        const patternBytes = bytes.map((byte, index) => (mask[index] ? byte : null));
        ...
        return this.kmpSearch(buffer, patternBytes, lps);
    }
}
