...
async function readOffsetsFromFile() {
    try {
        ...
        if (data === "" || !containsOffsets(data)) {
            console.error(
                chalk.red("You must actually have offsets in offsets.txt")
            );
            process.exit();
        }
        return data
...
function findClosestMatch(segment, patternBytes, firstCharacter) {
    ...
    for (let i = 0; i < segment.length - patternLength + 1; ) {
        ...
        const distance = patternDistance(
...
async function findOffsetsInNewLibrary(
    ...
    async function processOffset(
        ...
        try {
            ...
            const attemptOffset = async (searchStartIndex = 0) => {
                ...
                if (closestMatch) {
                    const newOffset =
                        currentNewLibraryData.indexOf(closestMatch);
                    ...
                    if (CHECK_TYPE) {
                        ...
                        if (oldType && newType) {
                            if (
                                ...
                                console.log(
                                    chalk.green("[TYPE_STATUS] - Passed ") +
                                        oldType.methodType,
                                    oldType.returnType +
                                        " => " +
                                        newType.methodType,
                            ...
                            console.error(
                                chalk.red(
                                    "[TYPE_STATUS] - Error fetching types"
                                )
                            );
                            return;
                        }
    ...
    if (LOGGING) {
        ...
        console.log(
            chalk.gray(
                `Total elapsed time: ${chalk.blue(elapsedTime.toFixed(2))}ms`
            )
        );
    }
...
async function main() {
    try {
        ...
        if (LOGGING) {
            ...
            console.log(
                chalk.gray(
                    `Total processing time: ${chalk.blue(elapsedTime)}ms`
                )
            );
        }
    } catch (error) {
