...
async function main() {
  try {
    ...
    const hex = "F5 53 BE A9 F3 7B 01 A9 95 C2 01 F0 A8 86 4A 39 F3 03 01 2A F4 03 00 AA 28 01 00 37 00 9A 01 D0 00 D8 43 F9 CE 70 36 97 00 9A 01 F0 00 84 47 F9 CB 70 36 97 28 00 80 52 A8 86 0A 39 E0 03 14 AA F6 FD FF 97 40 03 00 B4 08 38 40 F9 08 03 00 B4"
    ...
    const hexclosestMatch = await findClosestMatchByOffset(offsetpat, binaryData);
    console.log("closest match:\n" +
      " * Hex:", hexclosestMatch.toString("hex") + "\n" +
      " * offset:", await getMemoryOffset(hexclosestMatch.toString("hex"), filePath)
    );
  } catch (error) {
